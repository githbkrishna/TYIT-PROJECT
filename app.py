...
df = pd.read_csv("preprocessing_main_house_prediction_file.csv")
...
def house_price_prediction(model,bath,balcony,total_sqft_int,bhk,price_per_sqft,area_type,availability,location):
    ...
    x[0]=float(bath)
    x[1]=float(balcony)
    x[2]=float(total_sqft_int)
    x[3]=float(bhk)
    x[4]=float(price_per_sqft)
...
def predict():
    ...
    return render_template("index.html", prediction_text="Predicted price of bangalore House is {}lakhs ₹".format(predicated_price))
